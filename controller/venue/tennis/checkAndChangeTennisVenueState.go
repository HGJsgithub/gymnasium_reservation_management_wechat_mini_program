...
func CheckAndChangeTennisVenueState1(c *gin.Context) {
	tableName := "tennis_venue_state1"
	...
	for i := range timeField1 {
		...
		for row.Next() {
			...
			if s == true {
				fmt.Printf("%d号场地%s时间段已被预订！\n", id1, constData.BadmintonTimePeriod[timeField1[i]])
				msg[id1] = []string{}
				msg[id1] = append(msg[id1], constData.BadmintonTimePeriod[timeField1[i]])
		...
		for i := range timeField2 {
			...
			for row.Next() {
				...
				if s == true {
					fmt.Printf("%d号场地%s时间段已被预订！\n", id2, timeField2[i])
					msg[id2] = []string{}
					msg[id2] = append(msg[id2], constData.BadmintonTimePeriod[timeField2[i]])
				} else if success == true {
...
func CheckAndChangeTennisVenueState2(c *gin.Context) {
	tableName := "tennis_venue_state2"
	db := database.ConnectTOGormDB()
	count := c.PostForm("count")
	state, _ := strconv.ParseBool(c.PostForm("state"))
	var success = true
