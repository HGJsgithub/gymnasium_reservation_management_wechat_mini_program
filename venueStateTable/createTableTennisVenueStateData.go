...
func CreateTableTennisVenueStateData(db *gorm.DB, venueNum int) {
	for i := 0; i < venueNum; i++ {
		todayVenueState := venueModel.TableTennisVenueState{ID: i + 1, Date: "today"}
		tomorrowVenueState := venueModel.TableTennisVenueState{ID: i + 1, Date: "tomorrow"}
		if db.First(&todayVenueState).RecordNotFound() == true {
			db.Create(&venueModel.TableTennisVenueState{
				ID:   i + 1,
				Date: "today",
				T9:   false,
				T10:  false,
			...
			fmt.Println(i+1, "号场地今天的乒乓球场地已经存在！")
			fmt.Println()
		}
		if db.First(&tomorrowVenueState).RecordNotFound() == true {
			...
			fmt.Println(i+1, "号场地明天的乒乓球场地已经存在！")
			fmt.Println()
		}
	}
}
